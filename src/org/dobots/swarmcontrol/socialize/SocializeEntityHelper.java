...
//					updateRobotEntity(i_oContext, eRobot);
		...
		for (SwarmAction eAction : SwarmAction.values()) {
			if (eAction.isEnabled()) {
				if ((oEntity = getActionEntity(i_oContext, eAction)) == null) {
					createSwarmActionEntity(i_oContext, eAction);
				} else {
					// updateSwarmActionEntity(i_oContext, eAction);
				}
			}
		}
	...
	public static void updateMainEntity(BaseActivity i_oContext, Entity i_oEntity) {
		...
		try {
			...
			oMetaData.put("szsd_title", "Swarm Control v0.4");
			oMetaData.put("szsd_description", "The Swarm Control app lets you connect to your robots, display the robots' sensor data " +
					"and control them remotely with your phone. Moreover, the app can connect to multiple robots at the same " +
					"time to group them together and let them execute swarm actions. You can choose from a variety of robots such as " +
					"the iRobot Roomba, Lego Mindstorms NXT, Wowwee RoboScooper, Parrot AR Drone, Brookstone Rover AC13, Brookstone Rover 2.0 " +
					"Meccano Spykee, I-Spy Tank and our home made robot Dotty. The app is still under heavy developement so new robots and " +
					"swarm actions are continuously added. Check our website http://www.dobots.nl for up-to-date " +
					"information on the app and let us know what you think.");
//			oMetaData.put("szsd_continue_link", "http://www.dobots.nl/blog/-/blogs/swarm-control-update");
	...
	private static String getDescription(RobotType i_eType) {
		...
		switch (i_eType) {
		case RBT_AC13ROVER:
			return "Control the Brookstone AC13 Rover remotely and display the video " +
					"on the smartphone.";
		case RBT_DOTTY:
		...
		case RBT_SPYKEE:
			return "If your Meccano Spykee is connected to a WiFi network you can use this " +
					...
					"on the smartphone.";
		case RBT_ROMO:
			return "Equip your Romo with an Android phone, then run our Romo Brain app and use the " +
					"swarm control app to control the Romo remotely and display the phone's camera.";
		case RBT_ROVER2:
			return "Control the Brookstone Rover 2.0 remotely and display the video " +
					"on the smartphone.";
		case RBT_SPYTANK:
			return "Use the swarm control app to control the I-Spy Tank, drive it around, move the " +
					"camera up / down and display the video on the phone.";
		default:
			return "";
		}
	}
	...
	private static String getKey(SwarmAction eAction) {
		return eAction.name();
	}
	
	private static Entity createSwarmActionEntity(BaseActivity i_oContext, SwarmAction eAction) {
		String strKey = getKey(eAction);
		String strTitle = eAction.toString();
		String strDescription = getDescription(eAction);
		
		return createEntity(i_oContext, strKey, strTitle, strDescription, "");
	}
		
	private static String getDescription(SwarmAction eAction) {
		switch(eAction) {
		case sa_Dance:
			return "Create a Dance out of a set of moves like forward, backward and rotate " +
					"then connect all your robots and let them execute the dance together.";
		default:
			return "";
		}
	}
	...
	// --------------------------------------------------------------------------------------
	public static Entity getActionEntity(BaseActivity i_oContext, SwarmAction eAction) {
